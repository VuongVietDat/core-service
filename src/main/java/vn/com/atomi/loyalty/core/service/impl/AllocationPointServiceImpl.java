...
import vn.com.atomi.loyalty.core.repository.CountPeriodRepository;
...
public class AllocationPointServiceImpl extends BaseService implements AllocationPointService {
    ...
    private final CountPeriodRepository countPeriodRepository;
    ...
    public void handlerAllocationPointEvent(AllocationPointMessage allocationPointMessage) {
        ...
        switch (allocationPointMessage.getType()) {
            case TRANSACTION -> allocationPointTransactionEvent(
                    ...
                    customerInput,
                    customerBalance);
            case HOLIDAY -> allocationPointHolidayEvent(
                    allocationTransaction, dictionaries, rules, customerInput, customerBalance);
            case USER_INTRODUCTION -> allocationPointUserIntroductionEvent(
                    allocationTransaction, dictionaries, rules, customerInput, customerBalance);
            case APPLICATION_ACTIVITY -> allocationPointApplicationActivityEvent(
                    allocationTransaction, dictionaries, rules, customerInput, customerBalance);
            case HAPPY_DOB -> allocationPointHappyDOBEvent(
                    allocationTransaction, dictionaries, rules, customerInput, customerBalance);
        }
    }
...
//          var remain = ruleAllocationOutput.getLimitEventPerUser() - totalAccumulated;
                    var remain = ruleAllocationOutput.getLimitPointPerUser() - totalAccumulated;
                    if (remain <= 0) {
                        consumptionPoint = 0;
                    } else {
                        consumptionPoint = Math.min(consumptionPoint, remain);
    ...
    private boolean compareCondition(
        ...
        return switch (ruleConditionOutput.getProperties()) {
            ...
            // Loại tiền
            case CURRENCY -> !Objects.equals(ruleConditionOutput.getValue(), transactionInput.getCurrency());
            // Sinh nhật KH
            case DOB -> customerInput.getDob() != null
                    && !customerInput.getDob().isEqual(transactionInput.getTransactionAt().toLocalDate());
            // Loại khách hàng
            case CUSTOMER_TYPE -> !Objects.equals(ruleConditionOutput.getValue(), customerInput.getCustomerType());
            // Loại sản phẩm
            case PRODUCT_TYPE -> {
                List<String> values =
