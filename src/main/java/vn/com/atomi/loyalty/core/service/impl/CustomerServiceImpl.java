...
public class CustomerServiceImpl extends BaseService implements CustomerService {
    ...
    public CustomerOutput get(Long id) {
        var pointAccountProjection = customerRepository.findByDeletedFalseAndPointAccount(id);
        var genderOutPut = masterDataService.getDictionary(Constants.DICTIONARY_GENDER, false);
        var uniqueType = masterDataService.getDictionary(Constants.DICTIONARY_UNIQUE_TYPE, false);
        var customerType = masterDataService.getDictionary(Constants.DICTIONARY_CUSTOMER_TYPE, false);
        var nationality = masterDataService.getDictionary(Constants.DICTIONARY_NATIONALITY, false);
        var dictionaries = new ArrayList<>(genderOutPut);
        dictionaries.addAll(nationality);
        return customerRepository
                .findById(id)
                .map(customer -> modelMapper.convertToCustomerOutput(customer, dictionaries))
